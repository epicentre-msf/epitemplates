...
#'}
epi_render = function(path, output_format = "all", output_dir = NULL, apply_rules = TRUE, ...){

    if (length(path) > 1){
      lapply(path, epi_render, output_format, output_dir, apply_rules, ...)
      return(invisible())
    }
    ...
    if (fs::is_dir(path)) {
        ...
        if (apply_rules) apply_yml_rules(path, output_dir)
        return(invisible())
    }
